 ...
 * centered. Primary voice-input affordance for the the team portal.
 *
 * Gestures:
 ...
 * flags the next send as mode="voice", which both shortens Atlas's
  ...
  if (!supported) {
    return (
      <button
        ...
        className={`inline-flex ${sz.button} items-center justify-center rounded-full border-2 border-brand-slate-600/30 text-brand-slate-600/40 dark:text-brand-slate-50/40`}
  ...
  if (recorder.state === "uploading") {
    return (
      <button
        type="button"
        disabled
        className={`inline-flex ${sz.button} items-center justify-center rounded-full bg-brand-slate-600 text-white dark:bg-brand-slate-50 dark:text-brand-slate-900`}
    ...
    return (
      <div className="flex flex-col items-center gap-1">
        <button
          ...
          className={`relative inline-flex ${sz.button} items-center justify-center rounded-full bg-brand-red-600 text-white shadow-lg ring-4 ring-brand-red-600/30 focus:outline-none focus:ring-4 focus:ring-brand-red-600/40 disabled:opacity-60 dark:bg-brand-red-400`}
        ...
        <div className="flex items-center gap-2 text-xs font-medium text-brand-red-600 dark:text-brand-red-400">
          ...
          <button
            ...
            className="inline-flex items-center justify-center text-brand-red-600/70 hover:text-brand-red-600 dark:text-brand-red-400/70 dark:hover:text-brand-red-400"
  ...
  return (
    <div className="flex flex-col items-center gap-1">
      <button
        ...
        onPointerCancel={onPointerCancel}
        disabled={disabled}
        title="Tap to dictate (or press and hold on touch). Atlas will speak the reply."
        className={`inline-flex ${sz.button} items-center justify-center rounded-full bg-brand-slate-600 text-white shadow-lg transition hover:bg-brand-slate-700 focus:outline-none focus:ring-4 focus:ring-brand-amber-500/40 active:scale-95 disabled:cursor-not-allowed disabled:opacity-50 dark:bg-brand-slate-50 dark:text-brand-slate-900 dark:hover:bg-brand-slate-50/85`}
      ...
      {recorder.error ? (
        <span
          className={`${sz.errorMaxWidth} text-center text-xs text-brand-red-600 dark:text-brand-red-400`}
          role="alert"
        >
          {recorder.error}
        </span>

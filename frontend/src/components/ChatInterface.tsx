...
interface ContactGroup {
  ...
  truncated: boolean;
  limit: number;
  /** True if the search filtered by ex_partner — surfaces the pill. */
  showExGov: boolean;
...
export function ChatInterface() {
  ...
  return (
    <div className="flex h-full min-h-0 flex-col">
      ...
      >
        {bubbles.length === 0 && !sending ? (
          <p className="mx-auto mt-12 max-w-prose text-center text-sm italic opacity-60">
            Welcome to the Atlas team portal. Right now, our portal is just
        ...
        {sending ? (
          <div className="flex justify-start">
            <div
              className="inline-flex items-center gap-2 rounded-lg bg-brand-slate-50 px-4 py-2 text-sm italic opacity-70 dark:bg-brand-slate-800"
              role="status"
      ...
      <div className="border-t border-brand-slate-600/20 pt-3 dark:border-brand-slate-50/20">
        {hasMessages ? (
        ...
        <div className="flex items-center gap-2">
          <textarea
            ...
            rows={2}
            disabled={sending}
            className="flex-1 resize-none rounded border border-brand-slate-600/30 bg-white p-3 text-sm focus:border-brand-slate-600 focus:outline-none focus:ring-1 focus:ring-brand-amber-500 disabled:opacity-60 dark:border-brand-slate-50/30 dark:bg-brand-slate-800 dark:text-brand-slate-50"
          />
          <button
            ...
            onClick={send}
            disabled={!canSend}
            className={`inline-flex h-10 shrink-0 items-center justify-center rounded bg-brand-slate-600 px-6 text-xs font-bold uppercase tracking-wide text-white hover:bg-brand-slate-700 focus:outline-none focus:ring-2 focus:ring-brand-amber-500 disabled:cursor-not-allowed disabled:opacity-50 dark:bg-brand-slate-50 dark:text-brand-slate-900 dark:hover:bg-brand-slate-50/85 ${
        ...
        <div className="mt-1">
          <span
            className={`text-xs ${
              overLimit
                ? "font-bold text-brand-red-600 dark:text-brand-red-400"
                : "opacity-60"
            }`}
          >
  ...
  if (bubble.kind === "user") {
    return (
      <div className="flex justify-end">
        <div className="max-w-[85%] whitespace-pre-wrap rounded-lg bg-brand-slate-600 px-4 py-2 text-sm text-white dark:bg-brand-slate-50 dark:text-brand-slate-900">
          {bubble.content}
        </div>
      </div>
    );
  ...
  if (bubble.kind === "error") {
    return (
      <div className="flex justify-center">
        <div className="max-w-[85%] rounded border border-brand-red-600/50 bg-brand-red-600/5 px-4 py-2 text-center text-xs text-brand-red-600 dark:text-brand-red-400">
          {bubble.content}
        </div>
      </div>
    );
  ...
  return (
    <div className="flex justify-start">
      <div className="w-full max-w-[95%] space-y-3 sm:max-w-[85%]">
        ...
        {bubble.content ? (
          <div className="atlas-md rounded-lg bg-brand-slate-50 px-4 py-2 text-sm text-brand-slate-900 dark:bg-brand-slate-800 dark:text-brand-slate-50">
            <ReactMarkdown
